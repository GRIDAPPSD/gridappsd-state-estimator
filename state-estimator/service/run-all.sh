#!/bin/bash
ulimit -c unlimited # make sure core files are allowed to be generated
...
pushd .
cd ~/git/gridappsd-state-plotter/state-plotter; ./state-plotter.py $1 $SIMREQ >/dev/null &
popd
#bin/state-estimator.out $1 $SIMREQ >/dev/null 2>/dev/null
bin/state-estimator.out $1 $SIMREQ
