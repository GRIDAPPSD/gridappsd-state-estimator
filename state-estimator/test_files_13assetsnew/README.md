...
1. nodelist.csv: list of node names composed of the bus name separated by a period, ".", from the phase given by a number.

2. ysparse.csv: Bus Admittance Matrix, Ybus, in physical units. First line is "Row,Col,G,B" header. Row and column index values correspond with the row or line numbers for nodes in the nodelist.csv file.
...
4. regid.csv: For each regulator (transformer) given by a CIM mrid value, lists associated primary and regulator nodes. First line is "Regid,Primnode,Regnode" header. Primnode and Regnode values are the same as the names in nodelist.csv.
