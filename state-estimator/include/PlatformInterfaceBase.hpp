...
public:
  ...
  virtual void publishEstimate(const uint &timestamp, SDMAP &est_v,
                               SDMAP &est_angle, SDMAP &est_vvar,
                               SDMAP &est_anglevar, SDMAP &est_vmagpu,
                               SDMAP &est_vargpu) = 0;
  ...
  uint getsbase() { return *sbase_ref; }

  SLIST getnode_names() { return node_names; }

  SIMAP getnode_idxs() { return node_idxs; }

  ISMAP getnode_name_lookup() { return node_name_lookup; }

  IMMAP getYphys() { return Yphys; }

  SCMAP getVnoms() { return node_vnoms; }

  SensorArray getZary() { return Zary; }

  IMDMAP getAmat() { return Amat; }

  SSMAP getregid_primnode() { return regid_primnode; }

  SSMAP getregid_regnode() { return regid_regnode; }

  SSMAP getmmrid_pos_type() { return mmrid_pos_type; }

  SSMAP getswitch_node1s() { return switch_node1s; }

  SSMAP getswitch_node2s() { return switch_node2s; }

  uint getmeas_timestamp() { return meas_timestamp; }

  SLIST getmeas_mrids() { return meas_mrids; }

  SDMAP getmeas_magnitudes() { return meas_magnitudes; }

  SDMAP getmeas_angles() { return meas_angles; }

  SDMAP getmeas_values() { return meas_values; }

  SSMAP getnode_bmrids() { return node_bmrids; }

  SSMAP getnode_phs() { return node_phs; }
